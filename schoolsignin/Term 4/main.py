...
def on_double_click(event):
    ...
    def search_grade(id):
        top = Toplevel()
        info = database.GradeSelect(id).result
        tree = Treeview(top)
        ...
        tree.heading("one", text="math",anchor=W)
    ...
    Button(top, text="grades", command=lambda:search_grade(item["text"])).grid(row=6, column=0)
...
Label(g_insert, text="physycs").grid(row=2, column=0)
physics = StringVar()
...
Button(g_insert, text="create", command=grade_insert).grid(row=6, column=0,
